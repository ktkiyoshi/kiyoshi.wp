...
$uri_media=API_URL_MEDIA.'&count='.$limit;
$options['request_type'] = 0;
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $uri_media); /* 取得するURL */
curl_setopt($ch, CURLOPT_POST, $options['request_type']); /* TRUE:HTTP"POST"を行う */
if( !empty($options['post']) ) {
    curl_setopt($ch, CURLOPT_POSTFIELDS, $options['post']);/* HTTP"POST"で送信する全データ */
}
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE); /* FALSE:cURLはサーバ証明書の検証を行わない */
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); /* TRUE:curl_exec()の返り値を文字列で返す */
